...
const getNextPageParam = (lastPage) => (lastPage.hasNextPage ? lastPage.currentPage + 1 : null);

const getAllResults = (query) => query.data.value?.pages?.flatMap((p) => p.results) ?? [];

function fetchAllPagesWatcher(query) {
  const loading = query.isLoading.value || query.isFetching.value || query.isFetchingNextPage.value;
  if (!loading && query.isSuccess.value && query.hasNextPage.value) query.fetchNextPage();
}
...
export function useCategoriesQuery(fetchAllPages = false) {
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => (await CategoriesService.get(pageParam)).data,
    getNextPageParam,
  });
...
export function useToppingsQuery(fetchAllPages = false) {
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => (await ToppingsService.get(pageParam)).data,
    getNextPageParam,
  });
...
export function useProductsQuery(fetchAllPages = false) {
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => (await ProductsService.get(pageParam)).data,
    getNextPageParam,
  });
...
export function useOrdersQuery() {
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => (await OrdersService.get(pageParam)).data,
    getNextPageParam,
  });
